...
export default function NewMain() {
  ...
  const allImagesLoaded = Object.values(loadedImages).length === 2 && Object.values(loadedImages).every(status => status);

  return (
    <div>
      {!allImagesLoaded && (
        <div className="justify-center items-center flex">
          <img src={loading} alt="loading" className="w-9/12 mt-10 mb-20" />
        </div>
      )}
      <div className="items-center justify-center flex" style={{ display: allImagesLoaded ? "flex" : "none" }}>
        <img
          src={currentImg}
          alt="img"
